...
class LibriSpeechDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        # Frame stacking to the encoder's 30 ms rate (paper §6.2). Applied after
        # SpecAugment so masking still operates on the 80-bin mel spectrogram.
        stack = self.feature_cfg.get("frame_stack", 1)
        stride = self.feature_cfg.get("frame_stride", 1)
        if stack > 1 or stride > 1:
            from stream_features import stack_frames

            features = stack_frames(features, stack, stride)
...
class rnnt_collate:
    ...
    A class rather than a closure so that DataLoader workers can pickle it -- macOS
    spawns workers rather than forking, and a nested function is not picklable.
    """
    ...
    def __call__(self, batch):
        ...
        feature_lengths = torch.tensor([f.size(0) for f in features], dtype=torch.long)
        target_lengths = torch.tensor([t.size(0) for t in tokens], dtype=torch.long)
        padded = pad_sequence(tokens, batch_first=True, padding_value=self.blank_id)
        sos = torch.full((padded.size(0), 1), self.blank_id, dtype=torch.long)
        return (
            pad_sequence(features, batch_first=True),
            feature_lengths,
